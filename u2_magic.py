...
class MagicAndLimit:
    ...
    def this_time(self) -> int:  # 当前种子距离上次汇报的时间
        this_time = self.announce_interval - self.to['next_announce'] - 1
    ...
    def limit_speed(self):
        ...
        for self.to in self.torrents_info:
            ...
            if time() - self.this_time + 1 > self.to['last_get_time'] and f1 == 0:
                # 刚汇报完，更新上次汇报的上传量
            ...
            for tr in table.contents[1:]:
                ...
                for self.to in self.torrents_info:
                    ...
                    if 'date' in self.to and 'last_get_time' in self.to:
                        if time() - self.this_time + 1 > self.to['last_get_time']:
                            if 'true_uploaded' in self.to or 'last_announce_time' in self.to:
                                tmp_info.append(self.to)
                            if (
                                    self.to['total_uploaded']
                                    + self.byte(self.to.get('uploaded_before') or '0 B', -1)
                                    - self.byte(data['uploaded'], 1)
            ...
            for tr in filter(lambda _tr: 'nowrap' in str(_tr), table):
                if tr.get('bgcolor'):
                    ...
                    if 'true_uploaded' in self.to:
                        ...
                        actual_uploaded_byte = self.byte(self.to['true_uploaded'], 0) \
                                               + self.byte(self.to.get('uploaded_before_byte') or '0 B', 0)
                        if abs(actual_uploaded_byte - self.byte(self.to['uploaded'], 0)) < 1024 ** 3:
                            ...
                            self.print(f"Actual upload of torrent {self.to['tid']} is {show_size(actual_uploaded_byte)}")
