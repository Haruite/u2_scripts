...
logger.add(level='DEBUG', sink=log_path, encoding='utf-8')
...
def get_url(url):
    try:
        html = get(url, headers=headers, proxies=proxies)
...
class U2Web(metaclass=U2Meta):
    def __init__(self):
        self.members = [name for name, func in inspect.getmembers(self)] + ['members']
        self.keys = [key for key in dir(self) if key not in self.members]
        self.info = {}
        self.tr = None
        self.tr1 = None
        self.trs = None
    ...
    def incomplete(self):  # 是否曾经未完成
        return bool('incomplete' in self.trs)

    def completed(self):  # 是否曾经完成
        return bool('snatchhlc_finish' in self.trs)

    def auxseed(self):  # 是否曾经辅种
    ...
    def promotion(self):  # 上传下载比率
        ...
        for img in td.select('img') or []:
            if not [pro.update(data) for key, data in pro_dict_1.items() if key in img['class'][0]]:
                pro[{'arrowup': 'ur', 'arrowdown': 'dr'}[img['class'][0]]] = float(
                    img.next_element.text[:-1].replace(',', '.'))
    ...
    def filename(self):  # 种子内容文件名
        return self.tr1.a.text[5:-8]

    def author(self):  # 发布者 uid
    ...
    '''
    def descrption(self):  # 详细描述
        return self.tr1.bdo.text
    '''

    def hash(self):  # info_hash
        return self.tr1.tr.contents[-2].contents[1].strip()

    def progress(self):  # (包括做种者在内的) 平均进度
    ...
    def select_torrent(self):
        ...
        """
        if not any(word for word in [self.seeding, self.leeching]):  # 过滤下载中和做种中的种子
            if self.sticky:  # 顶置种子
                if download_sticky:
            ...
            try:
                for self.tr in self.torrent_page():
                    if self.select_torrent():
                        ...
                        client.reconnect()
                        client.core.add_torrent_file(f'[U2].{self.tid}.torrent', b64encode(content), {'add_paused': False})
                        '''
                        if eval_all_keys:  # 获取种子所有信息
                            for _key in self.keys:
                                self.__getattribute__(_key)
                        for key in list(self.info.keys()):  # 删掉空的键
                            if self.info[key] in [None, '', []]:
                                del self.info[key]
                        if mgdb:
...
if __name__ == '__main__':
    if mgdb:
        ...
        added = [info['tid'] for info in torrent_info]
        write_list('added')
